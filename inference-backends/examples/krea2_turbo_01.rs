use inference_backends::stablediffusioncpp::{
    Krea2TurboJob, SamplingMethod, Scheduler, StableDiffusionCppConfig, StableDiffusionEvent,
    StableDiffusionJob,
};
...
#[tokio::main]
async fn main() {
    for outfile in (0..=10).map(|n| format!("krea2_1_{:02}", n)) {
        generate(&outfile, 1024, 1024, r#"
